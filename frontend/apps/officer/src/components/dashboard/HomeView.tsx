...
export function HomeView({ onNavigateTab }: { onNavigateTab: (tab: any) => void }) {
  ...
  const [viewMode, setViewMode] = React.useState<'cockpit' | 'summary'>('cockpit');

  if (viewMode === 'cockpit') {
    return (
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        <div className="bg-[#071326] px-4 py-1.5 border-b border-[#142A4D] flex items-center justify-between text-xs z-20 flex-shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold text-white uppercase tracking-wider">
              Workspace View:
            </span>
            <div className="flex items-center bg-[#0d1f38] p-0.5 rounded border border-[#1b345c]">
              <button
                type="button"
                onClick={() => setViewMode('cockpit')}
                className="px-2.5 py-1 rounded text-[11px] font-bold bg-[#14548C] text-white shadow-sm flex items-center gap-1.5"
              >
                <Compass className="w-3.5 h-3.5" />
                <span>4-Pillar Reconciliation Cockpit (Live)</span>
              </button>
              <button
                type="button"
                onClick={() => setViewMode('summary')}
                className="px-2.5 py-1 rounded text-[11px] font-semibold text-[#8EAECF] hover:text-white transition-colors flex items-center gap-1.5"
              >
                <BarChart3 className="w-3.5 h-3.5" />
                <span>District Executive Summary</span>
              </button>
            </div>
          </div>

          <div className="text-[11px] text-[#7EA3CC] flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>PS-26014 Inter-Silo Truth Engine Active</span>
          </div>
        </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="bg-[#071326] px-4 py-1.5 border-b border-[#142A4D] flex items-center justify-between text-xs sticky top-0 z-20">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-bold text-white uppercase tracking-wider">
            Workspace View:
          </span>
          <div className="flex items-center bg-[#0d1f38] p-0.5 rounded border border-[#1b345c]">
            ...
            >
              <Compass className="w-3.5 h-3.5" />
              <span>4-Pillar Reconciliation Cockpit</span>
            </button>
            <button
              type="button"
              onClick={() => setViewMode('summary')}
