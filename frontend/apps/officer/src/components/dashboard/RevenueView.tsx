...
export function RevenueView() {
  ...
  const loadRevenueData = async () => {
    ...
    setError(null);
    try {
      const res = await fetchParcelMutations('TN-607-001-042');
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveMutation(selectedMutation.id, remarks);
    } catch {}
    console.log(`Mutation ${selectedMutation.mutation_id || selectedMutation.id} approved and entered into RoR register.`);
    setMutations((prev) =>
      prev.map((m) =>
        m.id === selectedMutation.id ? { ...m, status: 'APPROVED' } : m
      )
    );
    setSelectedMutation((prev) => (prev ? { ...prev, status: 'APPROVED' } : null));
  };

  const handleReject = async () => {
    if (!selectedMutation) return;
    try {
      await rejectMutation(selectedMutation.id, remarks || 'Discrepancy in sale deed boundaries');
    } catch {}
    console.log(`Mutation rejected. Notice dispatched to applicant.`);
    setMutations((prev) =>
      prev.map((m) =>
        m.id === selectedMutation.id ? { ...m, status: 'REJECTED' } : m
      )
    );
    setSelectedMutation((prev) => (prev ? { ...prev, status: 'REJECTED' } : null));
  };
        ...
        {selectedMutation ? (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-white border border-[#DCE3EA] rounded-md space-y-3">
              ...
              <div className="space-y-1 pt-2 border-t border-[#DCE3EA]">
                <label className="text-xs font-semibold text-[#16212E] block">
                  Officer Order & Sanction Remarks
                </label>
                <textarea
                  rows={2}
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  placeholder="Enter statutory sanction order reference under Section 10(1)…"
                  className="w-full p-2 bg-white border border-[#B9C5D1] rounded text-xs text-[#16212E] focus:outline-none focus:ring-2 focus:ring-[#14548C]"
                />
              </div>
          ...
          <div className="p-8 text-center text-xs text-[#4A5B6E] bg-white rounded-md border border-[#DCE3EA]">
            Select a mutation casework item to inspect RoR and sanction title.
          </div>
        )}
      </div>
    </div>
